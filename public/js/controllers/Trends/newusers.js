...
newusers.controller('NewUsersCtrl',['$scope','$rootScope','$q','JettyService','UtilitiesService','JSONService',function($scope, $rootScope, $q, JettyService, UtilitiesService, JSONService) {
        ...
        var VisitorsDayWiseList = function(){
            ...
            try
            {
                for(i=data.length-1; i>=0; i--) {
                        daywiselist = daywiselist.concat({"x":data[i].x, "y":data[i].y,
                         "z":(i && ((data[i].y-data[i-1].y)/(data[i-1].y))*100).toFixed(1),
                          "a":new_user_organic_data[i].y, "b":new_user_inorganic_data[i].y,
                           "c":nu_data[i].y, "d":data[i].y-nu_data[i].y, "e":uninstalls_data[i].y,
                           "f":GoalUser[i].y});
                }
    ...
    /* Source Tab */
        ...
        */
        var TopSourcesTrend = function() {
            ...
            try
            {
                for(i=0;i<$scope.top_sources.length && i<3 ;i++)
                {
                    var source = $scope.top_sources[i].x;
    ...
    /* Device Tab */
        ...
        */
        var TopDevicesTrend = function() {
            ...
            try
            {
                for(i=0;i<$scope.top_devices.length && i<3 ;i++) {
                    var device = $scope.top_devices[i].x;
                    var manu = $scope.top_devices[i].y;
        ...
        */
        var DevicesList = function(){
            ...
            var UModelsAllobj={};
            var promises=[];
            var promise1 = JettyService.cummulative(domain+'ModelsAll',app,$scope.endGap,$scope.noDays,3,type,timeZone).then(function(data) {
 ...
 var GetUModelData = function(){
    ...
    /* Location Tab */
        ...
        var CityNewUsersList = function() {
            var deferred = $q.defer();
            $scope.CityNewUsersListCSV = $scope.cities; 
            $scope.city_items = $scope.cities.length;   
            ...
            $scope.$watch('city_current_page + items_per_page4', function() {
                ...
                $scope.page_city_list = $scope.cities.slice(begin, end);
            });
            deferred.resolve();
            return deferred.promise;
        };
